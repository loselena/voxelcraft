...
const App: React.FC = () => {
      ...
      if (!isRightClick) {
        ...
        if (!heldItem) {
          ...
          if (!slotItem) {
            ...
            // Corrected: replaced 'remaining' with 'rem'
            setHeldItem(rem > 0 ? { ...heldItem, count: rem } : null);
  ...
  const updateVisibleChunks = useCallback((pos: THREE.Vector3) => {
    const cx = Math.floor(pos.x / CHUNK_SIZE); const cz = Math.floor(pos.z / CHUNK_SIZE); setPlayerPos(pos.clone());
    ...
    for (let x = -RENDER_DISTANCE; x <= RENDER_DISTANCE; x++) {
      for (let x = -RENDER_DISTANCE; x <= RENDER_DISTANCE; x++) {
        for (let z = -RENDER_DISTANCE; z <= RENDER_DISTANCE; z++) {
          const key = worldManager.getChunkKey(cx + x, cz + z); if (!worldManager.chunks.has(key)) worldManager.generateChunk(cx + x, cz + z);
          newVisible.push(key);
        }
      }
    }
  ...
  const handleResetWorld = useCallback((e: React.MouseEvent) => {
    ...
    setCustomBlueprints(DEFAULT_BLUEBYPRINTS);
